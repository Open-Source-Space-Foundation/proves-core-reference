...
def test_01_time_set(fprime_test_api: IntegrationTestAPI, start_gds):
    ...
    assert previously_set_time == pytest.approx(
        datetime.now(timezone.utc), abs=timedelta(seconds=30)
    )

    # Assert event time is within 30 seconds of curiosity landing
    assert event_time == pytest.approx(curiosity_landing, abs=timedelta(seconds=30))
