...
// Helper to create angular velocity vector from magnitude in deg/s
// Puts all magnitude on X axis for simplicity
std::array<double, 3> makeVelocity(double deg_per_sec) {
    const double PI = 3.14159265358979323846;
    double rad_per_sec = deg_per_sec * PI / 180.0;
    return {rad_per_sec, 0.0, 0.0};
}
...
TEST(StrategySelectorTest, IdleBelowLowerThreshold) {
    ...
    auto result = selector.fromAngularVelocity(makeVelocity(0.5));
...
TEST(StrategySelectorTest, HysteresisAboveMaxThreshold) {
    ...
    auto result = selector.fromAngularVelocity(makeVelocity(15.0));
...
TEST(StrategySelectorTest, BdotInActiveRange) {
    ...
    auto result = selector.fromAngularVelocity(makeVelocity(8.0));
...
TEST(StrategySelectorTest, DeadbandCycleBehavior) {
    ...
    auto res1 = selector.fromAngularVelocity(makeVelocity(0.5));
    ...
    auto res2 = selector.fromAngularVelocity(makeVelocity(3.0));
    EXPECT_EQ(res2, StrategySelector::IDLE);

    // 3. Exceed Upper(5.0)
    // 6.0 >= 5.0. Target becomes Lower(1.0).
    // Should switch to BDOT
    auto res3 = selector.fromAngularVelocity(makeVelocity(6.0));
...
TEST(StrategySelectorTest, BoundaryConditions) {
    ...
    EXPECT_EQ(selector.fromAngularVelocity(makeVelocity(10.0)), StrategySelector::HYSTERESIS);

    // Exact Lower Threshold (when target is lower) -> BDOT
    // 1.0 >= 1.0
    EXPECT_EQ(selector.fromAngularVelocity(makeVelocity(1.0)), StrategySelector::BDOT);

    // Force target to Upper by going low
    selector.fromAngularVelocity(makeVelocity(0.0));

    // Exact Upper Threshold (when target is upper) -> BDOT
    // 5.0 >= 5.0
    EXPECT_EQ(selector.fromAngularVelocity(makeVelocity(5.0)), StrategySelector::BDOT);
}
