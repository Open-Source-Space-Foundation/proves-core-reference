...
class OpcodeAckBridge:
    ...
    def __init__(self, yamcs_url: str, yamcs_instance: str, dictionary_path: str):
        ...
        # Connect to YAMCS.
        logger.info(
            f"Connecting to YAMCS at {yamcs_url}, instance={yamcs_instance!r}"
        )
    ...
    def _load_cmd_dict(dictionary_path: str) -> dict:
        ...
        id_dict, _, _ = loader.get_id_dict(dictionary_path)
        return id_dict
    ...
    def _yamcs_to_fprime_name(yamcs_name: str) -> Optional[str]:
        ...
        The XTCE generated by fprime-to-xtce uses the convention::

            /SpaceSystemName|TopologyName/TopologyName|componentName|CMD_MNEMONIC

        The corresponding F Prime fully-qualified name is::

            componentName.CMD_MNEMONIC

        Example::

            /ReferenceDeployment|ReferenceDeployment/ReferenceDeployment|cmdDisp|CMD_NO_OP
            → cmdDisp.CMD_NO_OP
        """
        # Take everything after the last '/' separator.
        last_segment = yamcs_name.rsplit("/", 1)[-1]
    ...
    def _on_cmd_history(self, cmdhist) -> None:
        ...
        with self._lock:
            ...
            if not any(entry[0] == cmd_id for entry in queue):
                ...
                logger.debug(
                    f"Tracking pending command: {fprime_name!r}  id={cmd_id}"
                )
    ...
    def _on_event(self, event) -> None:
        ...
        logger.info(
            f"OpCodeCompleted → opcode={opcode:#010x}  command={fprime_name!r}"
        )
