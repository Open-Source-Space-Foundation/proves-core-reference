...
class DataBudgetAnalyzer:
    ...
    def __init__(self, project_root: Path):
        ...
        self.instances: Dict[str, ComponentInstance] = {}  # instance_name -> ComponentInstance
    ...
    def find_fpp_files(self) -> List[Path]:
        ...
        deployment_dir = self.project_root / 'FprimeZephyrReference' / 'ReferenceDeployment'
    ...
    def parse_struct_definition(self, lines: List[str], start_idx: int) -> Optional[TypeDefinition]:
    ...
    def parse_enum_definition(self, lines: List[str], start_idx: int) -> Optional[TypeDefinition]:
    ...
    def parse_array_definition(self, lines: List[str], start_idx: int) -> Optional[TypeDefinition]:
        """Parse an array definition from FPP"""
        array_match = re.match(r'\s*array\s+(\w+)\s*=\s*\[(\d+)\]\s*([A-Za-z0-9_.]+)', lines[start_idx])
        if not array_match:
            return None
    ...
    def parse_telemetry_channel(self, line: str, component_name: str) -> Optional[TelemetryChannel]:
    ...
    def parse_component_file(self, fpp_path: Path):
        ...
        while i < len(lines):
            ...
            comp_match = re.match(r'\s*(?:passive|active|queued)\s+component\s+(\w+)', line)
            if comp_match:
            ...
            if 'struct' in line and '{' in line:
                ...
                if type_def:
                    ...
                    key = f"{self.current_module}.{type_def.name}" if self.current_module else type_def.name
    ...
    def parse_packet_file(self, fppi_path: Path):
        ...
        while i < len(lines):
            ...
            packet_match = re.match(r'packet\s+(\w+)\s+id\s+(\d+)\s+group\s+(\d+)\s*\{', line)
            if packet_match:
            ...
            elif current_packet and not line.startswith('#') and not line.startswith('}'):
                # Channel references look like: ReferenceDeployment.component.ChannelName
                channel_match = re.match(r'([A-Za-z0-9_.]+)\.([A-Za-z0-9_]+)\.([A-Za-z0-9_]+)', line)
                if channel_match:
    ...
    def generate_report(self, output_file: Optional[str] = None):
        ...
        total_tlm_bytes = sum(ch.size_bytes for ch in self.telemetry_channels.values() if ch.size_bytes is not None)
