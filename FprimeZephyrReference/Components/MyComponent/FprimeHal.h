...
class PicoHal : public RadioLibHal {
  public:
    ...
    unsigned long millis() override { return to_ms_since_boot(get_absolute_time()); }

    unsigned long micros() override { return to_us_since_boot(get_absolute_time()); }
    ...
    void noTone(uint32_t pin) override { multicore_reset_core1(); }
    ...
    void spiTransfer(uint8_t* out, size_t len, uint8_t* in) { spi_write_read_blocking(_spiChannel, out, in, len); }

    void yield() override { tight_loop_contents(); }
