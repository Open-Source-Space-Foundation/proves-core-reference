...
int16_t
SX128x::begin(float freq, float bw, uint8_t sf, uint8_t cr, uint8_t syncWord, int8_t pwr, uint16_t preambleLength) {
...
int16_t SX128x::beginFLRC(float freq,
                          uint16_t br,
                          uint8_t cr,
                          int8_t pwr,
...
void SX128x::setDio1Action(void (*func)(void)) {
    this->mod->hal->attachInterrupt(this->mod->hal->pinToInterrupt(this->mod->getIrq()), func,
                                    this->mod->hal->GpioInterruptRising);
}
...
int16_t SX128x::startReceive() {
    return (this->startReceive(RADIOLIB_SX128X_RX_TIMEOUT_INF, RADIOLIB_IRQ_RX_DEFAULT_FLAGS,
...
int16_t SX128x::readData(uint8_t* data, size_t len) {
    ...
    if ((irq & RADIOLIB_SX128X_IRQ_CRC_ERROR) ||
        ((irq & RADIOLIB_SX128X_IRQ_HEADER_ERROR) && !(irq & RADIOLIB_SX128X_IRQ_HEADER_VALID))) {
...
int16_t SX128x::setSpreadingFactor(uint8_t sf) {
    ...
    if (modem == RADIOLIB_SX128X_PACKET_TYPE_LORA) {
        uint8_t data = 0;
        if ((this->spreadingFactor == RADIOLIB_SX128X_LORA_SF_5) ||
            (this->spreadingFactor == RADIOLIB_SX128X_LORA_SF_6)) {
            data = 0x1E;
        } else if ((this->spreadingFactor == RADIOLIB_SX128X_LORA_SF_7) ||
                   (this->spreadingFactor == RADIOLIB_SX128X_LORA_SF_8)) {
            data = 0x37;
        } else {
            data = 0x32;
        }
...
int16_t SX128x::setPreambleLength(size_t preambleLength) {
    ...
    if ((modem == RADIOLIB_SX128X_PACKET_TYPE_LORA) || (modem == RADIOLIB_SX128X_PACKET_TYPE_RANGING)) {
        ...
        return (setPacketParamsGFSK(this->preambleLengthGFSK, this->syncWordLen, this->syncWordMatch, this->crcGFSK,
                                    this->whitening, this->packetType));
    }
...
int16_t SX128x::setSyncWord(const uint8_t* syncWord, uint8_t len) {
    ...
    // update sync word
    int16_t state =
        SX128x::writeRegister(RADIOLIB_SX128X_REG_SYNC_WORD_1_BYTE_4 + (5 - len), const_cast<uint8_t*>(syncWord), len);
    RADIOLIB_ASSERT(state);
...
int16_t SX128x::setSyncWord(uint8_t syncWord, uint8_t controlBits) {
    ...
    // update register
    const uint8_t data[2] = {(uint8_t)((syncWord & 0xF0) | ((controlBits & 0xF0) >> 4)),
...
int16_t SX128x::setCRC(uint8_t len, uint32_t initial, uint16_t polynomial) {
    ...
    if ((modem == RADIOLIB_SX128X_PACKET_TYPE_GFSK) || (modem == RADIOLIB_SX128X_PACKET_TYPE_FLRC)) {
        ...
        this->crcGFSK = len << 4;
        state = setPacketParamsGFSK(this->preambleLengthGFSK, this->syncWordLen, this->syncWordMatch, this->crcGFSK,
                                    this->whitening, this->packetType);
        RADIOLIB_ASSERT(state);
        ...
        // set initial CRC value
        const uint8_t data[] = {(uint8_t)((initial >> 16) & 0xFF), (uint8_t)((initial >> 8) & 0xFF),
...
int16_t SX128x::setAccessAddress(uint32_t addr) {
    ...
    // set the address
    const uint8_t addrBuff[] = {(uint8_t)((addr >> 24) & 0xFF), (uint8_t)((addr >> 16) & 0xFF),
...
size_t SX128x::getPacketLength(bool update, uint8_t* offset) {
    ...
    if ((getPacketType() == RADIOLIB_SX128X_PACKET_TYPE_LORA) &&
        (this->headerType == RADIOLIB_SX128X_LORA_HEADER_IMPLICIT)) {
...
RadioLibTime_t SX128x::calculateTimeOnAir(ModemType_t modem, DataRate_t dr, PacketConfig_t pc, size_t len) {
    switch (modem) {
        case (ModemType_t::RADIOLIB_MODEM_LORA): {
            ...
            N_symbol = (float)N_symbolPreamble + coeff1 + 8.0f +
                       ceilf((float)RADIOLIB_MAX((int16_t)(8 * len + N_bitCRC - coeff2 + N_symbolHeader), (int16_t)0) /
                             (float)coeff3) *
                           cr;
        ...
        case (ModemType_t::RADIOLIB_MODEM_FSK):
            return (
                (((float)(pc.fsk.crcLength * 8) + pc.fsk.syncWordLength + pc.fsk.preambleLength + (uint32_t)len * 8) /
                 (dr.fsk.bitRate / 1000.0f)));
...
RadioLibTime_t SX128x::getTimeOnAir(size_t len) {
    ...
    if (modem == RADIOLIB_SX128X_PACKET_TYPE_LORA) {
        ...
        uint16_t preambleLength =
            (this->preambleLengthLoRa & 0x0F) * (uint32_t(1) << ((this->preambleLengthLoRa & 0xF0) >> 4));
...
int16_t SX128x::stageMode(RadioModeType_t mode, RadioModeConfig_t* cfg) {
    ...
    switch (mode) {
        case (RADIOLIB_RADIO_MODE_RX): {
            ...
            if ((this->headerType == RADIOLIB_SX128X_LORA_HEADER_IMPLICIT) &&
                (getPacketType() == RADIOLIB_SX128X_PACKET_TYPE_LORA)) {
                state = setPacketParamsLoRa(this->preambleLengthLoRa, this->headerType, this->payloadLen, this->crcLoRa,
                                            this->invertIQEnabled);
                RADIOLIB_ASSERT(state);
            }
        ...
        case (RADIOLIB_RADIO_MODE_TX): {
            ...
            uint8_t modem = getPacketType();
            if (modem == RADIOLIB_SX128X_PACKET_TYPE_LORA) {
                state = setPacketParamsLoRa(this->preambleLengthLoRa, this->headerType, cfg->transmit.len,
                                            this->crcLoRa, this->invertIQEnabled);
            } else if ((modem == RADIOLIB_SX128X_PACKET_TYPE_GFSK) || (modem == RADIOLIB_SX128X_PACKET_TYPE_FLRC)) {
                state = setPacketParamsGFSK(this->preambleLengthGFSK, this->syncWordLen, this->syncWordMatch,
                                            this->crcGFSK, this->whitening, this->packetType, cfg->transmit.len);
            } else if (modem == RADIOLIB_SX128X_PACKET_TYPE_BLE) {
                state = setPacketParamsBLE(this->connectionState, this->crcBLE, this->bleTestPayload, this->whitening);
            } else {
                return (RADIOLIB_ERR_WRONG_MODEM);
            ...
            // set DIO mapping
            state = setDioIrqParams(RADIOLIB_SX128X_IRQ_TX_DONE | RADIOLIB_SX128X_IRQ_RX_TX_TIMEOUT,
...
int16_t SX128x::setPacketParamsGFSK(uint8_t preambleLen,
                                    uint8_t syncLen,
                                    uint8_t syncMatch,
                                    uint8_t crcLen,
                                    uint8_t whiten,
...
int16_t SX128x::setPacketMode(uint8_t mode, uint8_t len) {
    ...
    int16_t state = setPacketParamsGFSK(this->preambleLengthGFSK, this->syncWordLen, this->syncWordMatch, this->crcGFSK,
                                        this->whitening, mode, len);
    RADIOLIB_ASSERT(state);
...
bool SX128x::findChip(const char* verStr) {
    ...
    while ((i < 10) && !flagFound) {
        ...
        if (strncmp(verStr, version, 6) == 0) {
            ...
            RADIOLIB_DEBUG_BASIC_PRINTLN("SX128x not found! (%d of 10 tries) RADIOLIB_SX128X_REG_VERSION_STRING:",
                                         i + 1);
            RADIOLIB_DEBUG_BASIC_HEXDUMP(reinterpret_cast<uint8_t*>(version), 16, RADIOLIB_SX128X_REG_VERSION_STRING);
