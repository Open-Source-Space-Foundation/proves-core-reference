...
void SX1280::SetSleep(SleepParams_t sleepConfig) {
    uint8_t sleep = (sleepConfig.WakeUpRTC << 3) | (sleepConfig.InstructionRamRetention << 2) |
...
void SX1280::ForcePreambleLength(RadioPreambleLengths_t preambleLength) {
    this->WriteRegister(REG_LR_PREAMBLELENGTH,
                        (this->ReadRegister(REG_LR_PREAMBLELENGTH) & MASK_FORCE_PREAMBLELENGTH) | preambleLength);
}
...
void SX1280::GetPacketStatus(PacketStatus_t* packetStatus) {
    ...
    switch (packetStatus->packetType) {
        ...
        case PACKET_TYPE_RANGING:
            ...
            (status[1] < 128) ? (packetStatus->LoRa.SnrPkt = status[1] / 4)
                              : (packetStatus->LoRa.SnrPkt = ((status[1] - 256) / 4));
            break;
...
void SX1280::Calibrate(CalibrationParams_t calibParam) {
    uint8_t cal = (calibParam.ADCBulkPEnable << 5) | (calibParam.ADCBulkNEnable << 4) |
                  (calibParam.ADCPulseEnable << 3) | (calibParam.PLLEnable << 2) | (calibParam.RC13MEnable << 1) |
...
void SX1280::EnableManualGain(void) {
    this->WriteRegister(REG_ENABLE_MANUAL_GAIN_CONTROL,
                        this->ReadRegister(REG_ENABLE_MANUAL_GAIN_CONTROL) | MASK_MANUAL_GAIN_CONTROL);
...
void SX1280::SetManualGainValue(uint8_t gain) {
    this->WriteRegister(REG_MANUAL_GAIN_VALUE,
                        (this->ReadRegister(REG_MANUAL_GAIN_VALUE) & MASK_MANUAL_GAIN_VALUE) | gain);
}
...
void SX1280::SetRangingIdLength(RadioRangingIdCheckLengths_t length) {
    switch (GetPacketType(true)) {
        case PACKET_TYPE_RANGING:
            WriteRegister(REG_LR_RANGINGIDCHECKLENGTH,
                          ((((uint8_t)length) & 0x03) << 6) | (ReadRegister(REG_LR_RANGINGIDCHECKLENGTH) & 0x3F));
            break;
        default:
            break;
    }
...
double SX1280::GetRangingResult(RadioRangingResultTypes_t resultType) {
    ...
    switch (GetPacketType(true)) {
        case PACKET_TYPE_RANGING:
            ...
            WriteRegister(REG_LR_RANGINGRESULTCONFIG, (ReadRegister(REG_LR_RANGINGRESULTCONFIG) & MASK_RANGINGMUXSEL) |
                                                          ((((uint8_t)resultType) & 0x03) << 4));
            valLsb = ((ReadRegister(REG_LR_RANGINGRESULTBASEADDR) << 16) |
                      (ReadRegister(REG_LR_RANGINGRESULTBASEADDR + 1) << 8) |
                      (ReadRegister(REG_LR_RANGINGRESULTBASEADDR + 2)));
            ...
            switch (resultType) {
                case RANGING_RESULT_RAW:
                    ...
                    // The API provide BW in [Hz] so the implemented formula is complement2( register ) / bandwidth[Hz]
                    // * A, where A = 150 / (2^12 / 1e6) = 36621.09
...
uint8_t SX1280::GetRangingPowerDeltaThresholdIndicator(void) {
    ...
    WriteRegister(REG_LR_RANGINGRESULTCONFIG, (ReadRegister(REG_LR_RANGINGRESULTCONFIG) & MASK_RANGINGMUXSEL) |
                                                  ((((uint8_t)RANGING_RESULT_RAW) & 0x03) << 4));  // Select raw results
...
void SX1280::RangingSetFilterNumSamples(uint8_t num) {
    ...
    WriteRegister(REG_LR_RANGINGFILTERWINDOWSIZE,
                  (num < DEFAULT_RANGING_FILTER_SIZE) ? DEFAULT_RANGING_FILTER_SIZE : num);
}
