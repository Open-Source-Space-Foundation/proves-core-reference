...
#define FREQ_STEP ((double)(XTAL_FREQ / (double)(1 << 18)))
...
/*
 * \brief Register and mask controlling the enabling of manual gain control
 */
#define REG_ENABLE_MANUAL_GAIN_CONTROL 0x089F
#define MASK_MANUAL_GAIN_CONTROL 0x80

/*!
 * \brief Register and mask controlling the demodulation detection
 */
 ...
 * The former keep the radio in Rx or Tx mode, even after successful reception
 * or transmission. It should never generate Timeout interrupt.
 * The later let the radio enough time to make one reception or transmission.
...
typedef struct {
    ...
    struct {
         ...
         *
         * In GFSK modulation, the bit-rate and bandwidth are linked together. In this structure, its values are set
         ...
         *
         * In FLRC modulation, the bit-rate and bandwidth are linked together. In this structure, its values are set
         * using the same token.
         */
        struct {
         ...
         *
         * In BLE modulation, the bit-rate and bandwidth are linked together. In this structure, its values are set
         * using the same token.
         */
        struct {
            RadioGfskBleBitrates_t
...
typedef struct {
    ...
    struct {
        ...
        struct {
            ...
            RadioPacketLengthModes_t HeaderType;  //!< If the header is explicit, it will be transmitted in the GFSK
            ...
            RadioWhiteningModes_t Whitening;      //!< Usage of whitening on payload and CRC blocks plus header block if
                                                  //!< header type is variable
        } Gfsk;
        /*!
         * \brief Holds the LORA packet parameters
         */
        struct {
            uint8_t PreambleLength;  //!< The preamble length is the number of LORA symbols in the preamble. To set it,
                                     //!< use the following formula @code Number of symbols = PreambleLength[3:0] * (
                                     //!< 2^PreambleLength[7:4] ) @endcode
            RadioLoRaPacketLengthsModes_t
                HeaderType;  //!< If the header is explicit, it will be transmitted in the LORA packet. If the header is
                             //!< implicit, it will not be transmitted
        ...
        struct {
            ...
            RadioSyncWordRxMatchs_t
                SyncWordMatch;  //!< The synchronization correlator to use to check synchronization word
            RadioPacketLengthModes_t HeaderType;  //!< If the header is explicit, it will be transmitted in the FLRC
                                                  //!< packet. If the header is implicit, it will not be transmitted.
...
class SX1280 : public Radio {
  ...
  public:
     ...
     * radio = SX1280( mosi, miso, sclk, nss, busy, int1, int2, int3, rst, &callbacks );
     * radio.Init( );
     * radio.SetInterruptMode( );   // Optional. Driver default behavior
     *
     * while( true )
     * {
     *     // Do some applicative work
     ...
     *
     * \see PacketStatus_t for the description of available information
     *
     ...
     * \param [in]  offset        The address in FIFO where writing first byte (default = 0x00)
     */
