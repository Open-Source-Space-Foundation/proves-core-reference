...
def stop_burnwire(fprime_test_api: IntegrationTestAPI):
    ...
    fprime_test_api.assert_event(f"{burnwire}.BurnwireEndCount", timeout=30)
...
def test_01_start_and_stop_burnwire(fprime_test_api: IntegrationTestAPI, start_gds):
    ...
    fprime_test_api.assert_event(f"{burnwire}.SetBurnwireState", "ON", timeout=30)

    fprime_test_api.assert_event(f"{burnwire}.SafetyTimerState", timeout=30)
