...
GDS_COMMAND ?= $(UV_RUN) fprime-gds -n --dictionary $(ARTIFACT_DIR)/zephyr/fprime-zephyr-deployment/dict/ReferenceDeploymentTopologyDictionary.json --communication-selection uart --uart-device $(UART_DEVICE) --uart-baud 115200 --output-unframed-data --framing-selection authenticate-space-data-link
